...
void ReplaceHolder(std::string& originStr, std::vector<std::string>& params, int32_t containCount)
{
    ...
    while (std::regex_search(start, end, matchs, RESOURCE_APP_STRING_PLACEHOLDER)) {
        ...
        if (shortHolderType) {
            replaceContentStr = params[static_cast<uint32_t>(searchTime + containCount)];
        } else {
            replaceContentStr = params[static_cast<uint32_t>(StringUtils::StringToInt(pos) - 1 + containCount)];
        }

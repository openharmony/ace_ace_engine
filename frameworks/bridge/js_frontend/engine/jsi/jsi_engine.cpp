...
#include "bridge/js_frontend/engine/jsi/panda_dummy/jsi/ark_js_runtime.h"
    ...
    const shared_ptr<JsRuntime>& runtime, const shared_ptr<JsValue>& styleObj, JsCommandDomElementOperator& command)
{
    ...
    if (!styleObj || !styleObj->IsObject(runtime)) {
        LOGE("domStyle argument is not an object, style not found!");
    }
...
bool JsiEngineInstance::InitJsEnv(bool debugger_mode)
{
    ...
    if (!runtime_->Initialize(library_path)) {
...
bool JsiEngineInstance::FireJsEvent(const std::string& eventStr)
{
    ...
    JsiUtils::SetCurrentState(runtime_, JsErrorType::FIRE_EVENT_ERROR, instanceId_, "", runningPage_);
...
void JsiEngine::GetLoadOptions(std::string& optionStr, bool isMainPage, bool hasAppCode)
{
    ...
    if (mediaQuery) {
        ...
        renderOption->Put("width", SystemProperties::GetWidth());
        renderOption->Put("height", SystemProperties::GetHeight());
...
void JsiEngine::LoadJs(const std::string& url, const RefPtr<JsAcePage>& page, bool isMainPage)
{
    ...
    auto delegate = engineInstance_->GetFrontendDelegate();
    JsiUtils::SetCurrentState(runtime, JsErrorType::LOAD_JS_BUNDLE_ERROR, instanceId_, page->GetUrl(), page);
...
void JsiEngine::DestroyPageInstance(int32_t pageId)
{
    ...
    if (page) {
        JsiUtils::SetCurrentState(runtime, JsErrorType::DESTROY_PAGE_ERROR, instanceId_, page->GetUrl(), page);
    } else {
        JsiUtils::SetCurrentState(runtime, JsErrorType::DESTROY_PAGE_ERROR, instanceId_);
    }
    func->Call(runtime, global, argv, argv.size());
...
void JsiEngine::DestroyApplication(const std::string& packageName)
{
    ...
    JsiUtils::SetCurrentState(runtime, JsErrorType::DESTROY_APP_ERROR, instanceId_, "",

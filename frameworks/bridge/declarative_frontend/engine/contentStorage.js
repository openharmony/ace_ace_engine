...
class SubscriberManager {
    constructor() {
        ...
        this.nextFreeId_ = 0;
        aceConsole.error("SubscriberManager has been created.");
    }
     ...
     */
    dumpSubscriberInfo() {
        aceConsole.error("Dump of SubscriberManager +++ (sart)");
        for (let [id, subscriber] of this.subscriberById_) {
            aceConsole.error(`Id: ${id} -> ${subscriber['info'] ? subscriber['info']() : 'unknown'}`);
        }
        aceConsole.error("Dump of SubscriberManager +++ (end)");
    }
    MakeId() {
...
class SubscribaleAbstract {
    ...
    constructor() {
        ...
        aceConsole.error(`SubscribaleAbstract: construcstor done`);
    }
    /**
    * A subsclass must call this function whenever one of its properties has
    ...
    notifyPropertyHasChanged(propName, newValue) {
        aceConsole.error(`SubscribaleAbstract: notifyPropertyHasChanged '${propName}'.`);
    ...
    addOwningProperty(subscriber) {
        aceConsole.error(`SubscribaleAbstract: addOwningProperty: subscriber '${subscriber.id()}'.`);
    ...
    removeOwningPropertyById(subscriberId) {
        aceConsole.error(`SubscribaleAbstract: removeOwningProperty '${subscriberId}'.`);
...
function Observed(target) {
    ...
    var f = function (...args) {
        aceConsole.error(`New ${original.name}, gets wrapped inside ObservableObject proxy.`);
...
class SubscribableHandler {
    constructor(owningProperty) {
        ...
        aceConsole.error(`SubscribableHandler: construcstor done`);
    }
    addOwningProperty(subscriber) {
        aceConsole.error(`SubscribableHandler: addOwningProperty: subscriber '${subscriber.id()}'.`);
        this.owningProperties_.add(subscriber.id());
    }
    /*
    ...
    removeOwningPropertyById(subscriberId) {
        aceConsole.error(`SubscribableHandler: removeOwningProperty '${subscriberId}'.`);
        this.owningProperties_.delete(subscriberId);
    }
    notifyPropertyHasChanged(propName, newValue) {
        aceConsole.error(`SubscribableHandler: notifyPropertyHasChanged '${propName}'.`);
        var registry = SubscriberManager.Get();
        this.owningProperties_.forEach((subscribedId) => {
            var owningProperty = registry.get(subscribedId);
            if (owningProperty) {
    ...
    set(target, property, newValue) {
        switch (property) {
            ...
            default:
                ...
                aceConsole.error(`SubscribableHandler: set property '${property.toString()}' to new value'`);
...
class ObservedPropertyAbstract {
    ...
    subscribeMe(subscriber) {
        aceConsole.error(`ObservedPropertyAbstract[${this.id()}, '${this.info() || "unknown"}']: subscribeMe: Property new subscriber '${subscriber.id()}'`);
    ...
    */
    ...
    notifyHasChanged(newValue, isCrossWindow) {
        aceConsole.error(`ObservedPropertyAbstract[${this.id()}, '${this.info() || "unknown"}']: notifyHasChanged, notifying.`);
        var registry = SubscriberManager.Get();
    ...
    notifyPropertyRead() {
        aceConsole.error(`ObservedPropertyAbstract[${this.id()}, '${this.info() || "unknown"}']: propertyRead.`);
        var registry = SubscriberManager.Get();
        this.subscribers_.forEach((subscribedId) => {
            var subscriber = registry.get(subscribedId);
            if (subscriber) {
...
class ObservedPropertyObject extends ObservedPropertyObjectAbstract {
    ...
    hasChanged(newValue, isCrossWindow) {
        aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}']: hasChanged`);
    ...
    */
    setValueInternal(newValue) {
        if (typeof newValue !== 'object') {
            aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}'] new value is NOT an object. Application error. Ignoring set.`);
            return false;
        }
        this.unsubscribeFromOwningProperty();
        if (ObservedObject.IsObservedObject(newValue)) {
            aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}'] new value is an ObservedObject already`);
            ObservedObject.addOwningProperty(newValue, this);
            this.wrappedValue_ = newValue;
        }
        else if (newValue instanceof SubscribaleAbstract) {
            aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}'] new value is an SubscribaleAbstract, subscribiung to it.`);
            this.wrappedValue_ = newValue;
            this.wrappedValue_.addOwningProperty(this);
        }
        else {
            aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}'] new value is an Object, needs to be wrapped in an ObservedObject.`);
            this.wrappedValue_ = ObservedObject.createNew(newValue, this);
        }
        return true;
    }
    get() {
        aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}']: get`);
        this.notifyPropertyRead();
        return this.wrappedValue_;
    }
    set(newValue, isCrossWindow) {
        if (this.wrappedValue_ == newValue) {
            aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}']: set with unchanged value - ignoring.`);
            return;
        }
        aceConsole.error(`ObservedPropertyObject[${this.id()}, '${this.info() || "unknown"}']: set, changed`);
...
class ObservedPropertySimple extends ObservedPropertySimpleAbstract {
    ...
    hasChanged(newValue, isCrossWindow) {
        aceConsole.error(`ObservedPropertySimple[${this.id()}, '${this.info() || "unknown"}']: hasChanged`);
        this.notifyHasChanged(this.wrappedValue_, isCrossWindow);
    }
    /*
      actually update this.wrappedValue_
    ...
    */
    setValueInternal(newValue) {
        aceConsole.error(`ObservedPropertySimple[${this.id()}, '${this.info() || "unknown"}'] new value is of simple type`);
        this.wrappedValue_ = newValue;
    }
    get() {
        aceConsole.error(`ObservedPropertySimple[${this.id()}, '${this.info() || "unknown"}']: get returns '${JSON.stringify(this.wrappedValue_)}' .`);
        this.notifyPropertyRead();
        return this.wrappedValue_;
    }
    set(newValue, isCrossWindow) {
        if (this.wrappedValue_ == newValue) {
            aceConsole.error(`ObservedPropertySimple[${this.id()}, '${this.info() || "unknown"}']: set with unchanged value - ignoring.`);
            return;
        }
        aceConsole.error(`ObservedPropertySimple[${this.id()}, '${this.info() || "unknown"}']: set, changed from '${JSON.stringify(this.wrappedValue_)}' to '${JSON.stringify(newValue)}.`);
        this.setValueInternal(newValue);
        this.notifyHasChanged(newValue, isCrossWindow);
    }
    /**
    ...
    */
    aboutToBeDeleted() {
        ...
        if (this.contentStoragelinkedParentProperty_) {
            this.contentStoragelinkedParentProperty_.unlinkSuscriber(this.id__());
        }
    ...
    hasChanged(newValue, isCrossWindow) {
        aceConsole.error(`SynchedPropertyObjectTwoWay[${this.id()}, '${this.info() || "unknown"}']: contained ObservedObject hasChanged'.`);
        this.notifyHasChanged(this.getObject(), isCrossWindow);
    }
    // get 'read through` from the ObservedProperty
    get() {
        aceConsole.error(`SynchedPropertyObjectTwoWay[${this.id()}, '${this.info() || "unknown"}']: get`);
        return this.getObject();
    }
    // set 'writes through` to the ObservedProperty
    set(newValue, isCrossWindow) {
        if (this.getObject() == newValue) {
            aceConsole.error(`SynchedPropertyObjectTwoWay[${this.id()}IP, '${this.info() || "unknown"}']: set with unchanged value '${newValue}'- ignoring.`);
            return;
        }
        aceConsole.error(`SynchedPropertyObjectTwoWay[${this.id()}, '${this.info() || "unknown"}']: set to newValue: '${newValue}'.`);
        ObservedObject.removeOwningProperty(this.getObject(), this);
...
class SynchedPropertyNesedObject extends ObservedPropertyObjectAbstract {
    ...
    */
    ...
    // will call this cb function when property has changed
    hasChanged(newValue, isCrossWindow) {
        aceConsole.error(`SynchedPropertyNesedObject[${this.id()}, '${this.info() || "unknown"}']: contained ObservedObject hasChanged'.`);
        this.notifyHasChanged(this.obsObject_, isCrossWindow);
    }
    // get 'read through` from the ObservedProperty
    get() {
        aceConsole.error(`SynchedPropertyNesedObject[${this.id()}, '${this.info() || "unknown"}']: get`);
        this.notifyPropertyRead();
    ...
    set(newValue, isCrossWindow) {
        if (this.obsObject_ == newValue) {
            aceConsole.error(`SynchedPropertyNesedObject[${this.id()}IP, '${this.info() || "unknown"}']: set with unchanged value '${newValue}'- ignoring.`);
            return;
        }
        aceConsole.error(`SynchedPropertyNesedObject[${this.id()}, '${this.info() || "unknown"}']: set to newValue: '${newValue}'.`);
...
class SynchedPropertySimpleOneWay extends ObservedPropertySimpleAbstract {
    ...
    */
    ...
    // get 'read through` from the ObservedProperty
    get() {
        aceConsole.error(`SynchedPropertySimpleOneWay[${this.id()}, '${this.info() || "unknown"}']: get returns '${this.wrappedValue_}'`);
        this.notifyPropertyRead();
        return this.wrappedValue_;
    }
    set(newValue, isCrossWindow) {
        if (this.wrappedValue_ == newValue) {
            aceConsole.error(`SynchedPropertySimpleOneWay[${this.id()}, '${this.info() || "unknown"}']: set with unchanged value '${this.wrappedValue_}'- ignoring.`);
            return;
        }
        aceConsole.error(`SynchedPropertySimpleOneWay[${this.id()}, '${this.info() || "unknown"}']: set from '${this.wrappedValue_} to '${newValue}'.`);
        this.wrappedValue_ = newValue;
        this.notifyHasChanged(newValue, isCrossWindow);
...
class SynchedPropertySimpleOneWaySubscribing extends SynchedPropertySimpleOneWay {
    ...
    hasChanged(newValue, isCrossWindow) {
        aceConsole.error(`SynchedPropertySimpleOneWaySubscribing[${this.id()}, '${this.info() || "unknown"}']: source property hasChanged'.`);
...
class SynchedPropertySimpleTwoWay extends ObservedPropertySimpleAbstract {
  ...
  */
    ...
    hasChanged(newValue, isCrossWindow) {
        aceConsole.error(`SynchedPropertySimpleTwoWay[${this.id()}, '${this.info() || "unknown"}']: hasChanged to '${newValue}'.`);
        this.notifyHasChanged(newValue, isCrossWindow);
    }
    // get 'read through` from the ObservedProperty
    get() {
        aceConsole.error(`SynchedPropertySimpleTwoWay[${this.id()}IP, '${this.info() || "unknown"}']: get`);
        this.notifyPropertyRead();
    ...
    set(newValue) {
        if (this.contentObserver_) {
            if (this.contentObserver_.get() == newValue) {
                aceConsole.error(`SynchedPropertySimpleTwoWay[${this.id__()}IP, '${this.info() || "unknown"}']: set with unchanged value '${newValue}'- ignoring.`);
                return;
            }
        ...
        if (this.source_.get() == newValue) {
            aceConsole.error(`SynchedPropertySimpleTwoWay[${this.id()}IP, '${this.info() || "unknown"}']: set with unchanged value '${newValue}'- ignoring.`);
            return;
        }
...
class ContentStorage {
    ...
    setOrCreate(propName, newValue, linkUser) {
        var p = this.storage_.get(propName);
        if (p) {
            aceConsole.error(`ContentStorage.setOrCreate(${propName}, ${newValue}) update existing property`);
            p.set(newValue);
        }
        else {
            aceConsole.error(`ContentStorage.setOrCreate(${propName}, ${newValue}) create new entry and set value`);
            const newProp = (typeof newValue === "object") ?
                new ObservedPropertyObject(newValue, linkUser, propName)
                : new ObservedPropertySimple(newValue, linkUser, propName);
    ...
    delete(propName) {
        ...
        if (p) {
            ...
            aceConsole.error(`Attempt to delete unknown property ${propName}.`);
            return false;
        }
    }
    keys() {
    ...
    clear() {
        ...
        aceConsole.error(`ContentStorage.deleteAll: success`);
    }

...
#include "frameworks/bridge/declarative_frontend/engine/jsi/panda_dummy/jsi/ark_js_runtime.h"
...
// -----------------------
JsiValue::JsiValue(panda::Local<panda::JSValueRef> rhs) : JsiType(rhs) {}
...
JsiRef<JsiValue> JsiArray::GetValueAt(size_t index) const
{
    ...
    return JsiRef<JsiValue>::Make(panda::ArrayRef::GetValueAt(vm, panda::Local<JSValueRef>(GetHandle()), index));
}
...
void JsiArray::SetValueAt(size_t index, JsiRef<JsiValue> value) const
{
    auto runtime = std::static_pointer_cast<ArkJSRuntime>(JsiDeclarativeEngineInstance::GetJsRuntime());
    auto vm = runtime->GetEcmaVm();
    panda::ArrayRef::SetValueAt(vm, panda::Local<panda::JSValueRef>(GetHandle()), index,
        panda::Local<panda::JSValueRef>(value.Get().GetHandle()));
}
...
JsiObject JsiObject::New()
{
    auto runtime = std::static_pointer_cast<ArkJSRuntime>(JsiDeclarativeEngineInstance::GetJsRuntime());
    return JsiObject(panda::ObjectRef::New(runtime->GetEcmaVm()));
}
...
{
    auto runtime = std::static_pointer_cast<ArkJSRuntime>(JsiDeclarativeEngineInstance::GetJsRuntime());
    return panda::FunctionRef::New(const_cast<panda::EcmaVM*>(runtime->GetEcmaVm()), func, nullptr);
}

// -----------------------
// Implementation of JsiObjectTemplate
// -----------------------
JsiObjTemplate::JsiObjTemplate(panda::Local<panda::JSValueRef> val) : JsiType(val) {}

void JsiObjTemplate::SetInternalFieldCount(int32_t count) const {}

JsiRef<JsiObject> JsiObjTemplate::NewInstance() const
{
    return JsiRef<JsiObject>::Make();
}

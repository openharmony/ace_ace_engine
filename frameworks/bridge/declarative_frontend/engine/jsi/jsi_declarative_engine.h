...
public:
    ...
    static void RootViewHandle(JsRuntime* runtime, shared_ptr<JsValue>& value);
    ...
    static RefPtr<JsAcePage> GetRunningPage(JsRuntime* runtime);
    static RefPtr<JsAcePage> GetStagingPage(JsRuntime* runtime);
    static void PostJsTask(JsRuntime* runtime, std::function<void()>&& task);
    static void TriggerPageUpdate(JsRuntime* runtime);
    static RefPtr<PipelineContext> GetPipelineContext(JsRuntime* runtime);
...
private:
    ...
    static thread_local std::unordered_map<int32_t, shared_ptr<JsValue>&> rootViewMap_;

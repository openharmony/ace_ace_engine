...
void JSGridContainer::Create(const JSCallbackInfo& info)
{
    ...
    if (info.Length() > 0 && info[0]->IsObject()) {
        ...
        int32_t columnNum;
        if (columns->IsNumber()) {
            columnNum = columns->ToNumber<int32_t>();
            gridContainerInfoBuilder.SetColumns(columnNum);
        }
        ...
        Dimension dim;
        // gutter?: Length
        if (JSViewAbstract::ToDimension(obj->GetProperty("gutter"), dim)) {
            gridContainerInfoBuilder.SetGutterWidth(dim);
        }

        // margin?: Length
        if (JSViewAbstract::ToDimension(obj->GetProperty("margin"), dim)) {

...
public:
    ...
    void RegiterDataChangeListener(const RefPtr<V2::DataChangeListener>& listener) override
    {
        if (!listener) {
            return;
        }
        ...
        listenerProxy = Referenced::Claim(listenerObj->Unwrap<JSDataChangeListener>());
        listenerProxy->AddListener(listener);
    ...
    void UnregiterDataChangeListener(const RefPtr<V2::DataChangeListener>& listener) override
    {
        if (!listener) {
            return;
        }
...
private:
    std::list<RefPtr<Component>>& ExpandChildren() override
    {
        ...
        if (!Expanded()) {
            ...
            RegiterDataChangeListener(defaultListener_);
        }
...
void JSLazyForEach::Create(const JSCallbackInfo& info)
{
    ...
    JSRef<JSFunc> keyGenerator = JSRef<JSFunc>::Cast(params[PARAM_KEY_GENERATOR]);
    ...
    if (keyGenerator.IsEmpty()) {
        ...
        keyGenFunc = [viewId, keyGenerator](const JSRef<JSVal>& jsVal, size_t index) {

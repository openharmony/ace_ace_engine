...
bool ConvertFromJSValue(const JSRef<JSVal>& jsValue, T& result)
{
    if constexpr (std::is_same_v<T, bool>) {
        ...
        if (jsValue->IsNumber()) {
            result = Dimension(jsValue->ToNumber<double>(), DimensionUnit::VP);
            return true;
        } else if (jsValue->IsString()) {
            result = StringUtils::StringToDimension(jsValue->ToString(), true);
            return true;
        }
    } else if constexpr (std::is_same_v<T, Color>) {
        if (jsValue->IsNumber()) {
            result.SetValue(jsValue->ToNumber<uint32_t>());
            return true;
        } else if (jsValue->IsString()) {

...
void JSTabContent::Create()
{
    ...
    if (component) {
        ...
        ViewStackProcessor::GetInstance()->Push(tabContent);
    }
}
...
{
    auto tabContent =
        AceType::DynamicCast<OHOS::Ace::TabContentComponent>(ViewStackProcessor::GetInstance()->GetMainComponent());
    if (!tabContent) {
        return;
    }
    auto parent = tabContent->GetParent().Upgrade();
    RefPtr<TabsComponent> tabs;
    while (parent) {
        tabs = AceType::DynamicCast<TabsComponent>(parent);
        if (tabs) {
            ProcessTabBarData(tabs, info);
            break;
        }
...
void JSTabContent::ProcessTabBarData(const RefPtr<TabsComponent>& tabs, const JSCallbackInfo& info)
{

...
void JSPageTransition::Translate(const JSCallbackInfo& info)
{
    ...
    if (info.Length() > 0 && info[0]->IsObject()) {
        ...
        JSViewAbstract::GetDimension("x", args, length);
        if (length) {
            translateX = length.value();
            length.reset();
        }
        JSViewAbstract::GetDimension("y", args, length);
        if (length) {
            translateY = length.value();
            length.reset();
        }
        JSViewAbstract::GetDimension("z", args, length);
        if (length) {
...
void JSPageTransition::Scale(const JSCallbackInfo& info)
{
    ...
    if (info.Length() > 0 && info[0]->IsObject()) {
        ...
        auto scaleX = static_cast<float>(args->GetDouble("x", 1.0f));
        auto scaleY = static_cast<float>(args->GetDouble("y", 1.0f));
        auto scaleZ = static_cast<float>(args->GetDouble("z", 1.0f));
        ...
        // if specify centerX
        std::optional<Dimension> length;
        JSViewAbstract::GetDimension("centerX", args, length);
        if (length) {
            centerX = length.value();
            length.reset();
        }
        // if specify centerY
        JSViewAbstract::GetDimension("centerY", args, length);
        if (length) {
...
void JSPageTransition::Opacity(const JSCallbackInfo& info)
{
    ...
    if (info.Length() > 0 && info[0]->IsNumber()) {
        auto opacity = info[0]->ToNumber<float>();

...
void JSRect::SetRadiusWithJsVal(const RefPtr<T>& component, const JSRef<JSVal>& jsVal)
{
    ...
    Dimension value = ToDimension(jsVal);
    component->SetRadiusWidth(value, option);
    component->SetRadiusHeight(value, option);
}
...
void JSRect::SetRadiusWithArrayValue(const RefPtr<T>& component, const JSRef<JSVal>& jsVal)
{
    ...
    for (int32_t i = 0; i < length; i++) {
        ...
        if (radiusX->IsNumber()) {
            radiusXValue = Dimension(radiusX->ToNumber<double>(), DimensionUnit::VP);
        }
        if (radiusX->IsString()) {
            radiusXValue = StringUtils::StringToDimension(radiusX->ToString());
        }
        Dimension radiusYValue;
        if (radiusY->IsNumber()) {
            radiusYValue = Dimension(radiusY->ToNumber<double>(), DimensionUnit::VP);
        }
        if (radiusY->IsString()) {
            radiusYValue = StringUtils::StringToDimension(radiusY->ToString());
        }
...
void JSRect::ConstructorCallback(const JSCallbackInfo& info)
{
    ...
    if (info.Length() > 0 && info[0]->IsObject()) {
        JSRef<JSObject> obj = JSRef<JSObject>::Cast(info[0]);
        Dimension width = ToDimension(obj->GetProperty("width"));
        if (width.IsValid()) {
            rect->SetWidth(width);
        }
        Dimension height = ToDimension(obj->GetProperty("height"));
        if (height.IsValid()) {
            rect->SetHeight(height);
        }
        Dimension radiusWidth = ToDimension(obj->GetProperty("radiusWidth"));
        if (radiusWidth.IsValid()) {
            rect->SetRadiusWidth(radiusWidth);
        }
        Dimension radiusHeight = ToDimension(obj->GetProperty("radiusHeight"));
        if (radiusHeight.IsValid()) {

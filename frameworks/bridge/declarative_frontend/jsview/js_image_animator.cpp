...
void JSImageAnimator::ParseImages(const std::unique_ptr<JsonValue>& image, ImageProperties& imageProperties)
{
    ...
    imageProperties.width = GetDimension("width", image);
    imageProperties.height = GetDimension("height", image);
    imageProperties.top = GetDimension("top", image);
    imageProperties.left = GetDimension("left", image);

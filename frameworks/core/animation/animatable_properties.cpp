                ...
                animation->AddListener(
                    [weak = AceType::WeakClaim(this), setter = setter, weakAnimation](const RefPtr<Animatable>& data) {
                        ...
                        if (animatableProperties && margin) {
                            if (animation && animation->GetInit() == nullptr) {
                                auto init = AceType::MakeRefPtr<AnimatableData<Dimension>>(
                                    animatableProperties->GetMargin(setter));
                                animation->SetInit(init);
                            }
                            animatableProperties->SetMargin(margin->GetValue(), setter);
                        }
                    });
                break;
            }
                ...
                animation->AddListener(
                    [weak = AceType::WeakClaim(this), setter = setter, weakAnimation](const RefPtr<Animatable>& data) {
                        ...
                        if (animatableProperties && padding) {
                            if (animation && animation->GetInit() == nullptr) {
                                auto init = AceType::MakeRefPtr<AnimatableData<Dimension>>(
                                    animatableProperties->GetPadding(setter));
                                animation->SetInit(init);
                            }
                            animatableProperties->SetPadding(padding->GetValue(), setter);
                        }
                    });
                break;
            }
                ...
                animation->AddListener(
                    [weak = AceType::WeakClaim(this), weakAnimation](const RefPtr<Animatable>& data) {
                        ...
                        if (animatableProperties && radius) {
                            if (animation && animation->GetInit() == nullptr) {
                                auto init =
                                    AceType::MakeRefPtr<AnimatableData<float>>(animatableProperties->GetBlurRadius());
                                animation->SetInit(init);
                            }
                            animatableProperties->SetBlurRadius(radius->GetValue());
                        }
                    });
                break;
            }
                        ...
                        if (animatableProperties && radius) {
                            if (animation && animation->GetInit() == nullptr) {
                                auto init = AceType::MakeRefPtr<AnimatableData<float>>(
                                    animatableProperties->GetBackdropRadius());
                                animation->SetInit(init);
                            }
                            animatableProperties->SetBackdropRadius(radius->GetValue());
                        }
                    });
                break;
            }
...
void AnimatableProperties::SetPadding(const Dimension& value, const DimensionHelper& helper)
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return;
...
Dimension AnimatableProperties::GetPadding(const DimensionHelper& helper)
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return {};
...
void AnimatableProperties::SetMargin(const Dimension& value, const DimensionHelper& helper)
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return;
...
Dimension AnimatableProperties::GetMargin(const DimensionHelper& helper) const
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return {};
...
void AnimatableProperties::SetBlurRadius(float radius)
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return;
...
float AnimatableProperties::GetBlurRadius() const
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return 0.0f;
...
void AnimatableProperties::SetBackdropRadius(float radius)
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return;
...
float AnimatableProperties::GetBackdropRadius() const
{
    auto renderNode = AceType::DynamicCast<RenderBox>(this);
    if (renderNode == nullptr) {
        return 0.0f;

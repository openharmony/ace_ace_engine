...
public:
    TransformSaver(flutter::Canvas* canvas, Offset offset)
    {
        if (canvas != nullptr) {
            canvas->save();
...
void FlutterRenderBox::Paint(RenderContext& context, const Offset& offset)
{
    ...
    std::unique_ptr<TransformSaver> alignTransformSaver;
    if (isUseAlign_ && !alignOffset_.IsZero()) {
        ...
        alignTransformSaver = std::make_unique<TransformSaver>(canvas, alignOffset_);
    }

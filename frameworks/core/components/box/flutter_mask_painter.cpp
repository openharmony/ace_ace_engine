...
void FlutterMaskPainter::LoadSVGImage(const WeakPtr<PipelineContext>& context)
{
    ...
    auto sharedImageManager = pipelineContext->GetSharedImageManager();
    imageProvider_ = AceType::MakeRefPtr<ImageProvider>(file_, sharedImageManager);
    if (imageProvider_->Invalid()) {
        maskLoadStatus_ = MaskLoadingStatus::LOAD_FAIL;
        LOGE("Create imageProvider fail! mask image is %{private}s", maskImage_.c_str());
        return;
    }
    ...
    imageProvider_->GetSVGImageDOMAsync(file_, successCallback, failedCallback, context);
}

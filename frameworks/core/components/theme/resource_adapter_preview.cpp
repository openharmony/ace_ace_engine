...
void ResourceAdapterPreview::Init(const ResourceInfo& resourceInfo)
{
    sysResDir_ = resourceInfo.GetPackagePath();
    LOGI("loading system resources from %{private}s", sysResDir_.c_str());
    ...
    UpdateConfig(resourceInfo.GetResourceConfiguration());
...
void ResourceAdapterPreview::UpdateConfig(const ResourceConfiguration& config)
{
    LOGI("UpdateConfig color=%{public}d", config.GetColorMode());
    sysResFloat_.clear();
    sysResColor_.clear();
    ...
    if (config.GetColorMode() == ColorMode::DARK) {
...
std::vector<std::string> ResourceAdapterPreview::GetStringArray(uint32_t resId) const
{
    return {};
}

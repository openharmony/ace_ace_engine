...
bool ThemeConstants::GetMediaResource(uint32_t resId, std::ostream& dest) const
{
    if (!resAdapter_) {
        return false;
    }
...
void ThemeConstants::LoadCustomStyle(const RefPtr<AssetManager>& assetManager)
{
    ...
    if (!AceApplicationInfo::GetInstance().GetFiles(STYLES_FOLDER_PATH, files)) {
    ...
    priorityFileList = AceApplicationInfo::GetInstance().GetStyleResourceFallback(fileNameList);

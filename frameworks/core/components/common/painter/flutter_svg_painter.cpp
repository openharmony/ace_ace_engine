...
void FlutterSvgPainter::SetFillStyle(SkPaint& skPaint, const FillState& fillState, uint8_t opacity, bool antiAlias)
{
    ...
    if (fillState.GetGradient()) {
        SetGradientStyle(skPaint, fillState);
    } else {
        skPaint.setColor(fillState.GetColor().GetValue());
    }
    skPaint.setAlphaf(fillState.GetOpacity().GetValue() * opacity * (1.0f / UINT8_MAX));
}
...
void FlutterSvgPainter::SetGradientStyle(SkPaint& skPaint, const FillState& fillState)
{
    ...
    for (const auto& gradientColor : gradientColors) {
        ...
        colors.push_back(gradientColor.GetColor().GetValue());
    }
    ...
    bool antiAlias)
{
    ...
    skPaint.setAlphaf(strokeState.GetOpacity().GetValue() * opacity * (1.0f / UINT8_MAX));
    skPaint.setAntiAlias(antiAlias);

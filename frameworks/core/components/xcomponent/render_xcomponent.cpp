...
{
    if (!NeedLayout()) {
        LOGI("RenderXCopmponent::PerformLayout No Need to Layout");
        return;
    }
...
void RenderXComponent::MarkNeedRender(bool overlay)
{
...
{
    if (delegate_ && (delegate_->GetId() != INVALID_ID) &&
...
void RenderXComponent::CallXComponentLayoutMethod()
{
    ...
    if (NearEqual(prePosition_.GetX(), position_.GetX()) && NearEqual(prePosition_.GetY(), position_.GetY())) {
        LOGE("xcomponent position do not update");
        return;
    }

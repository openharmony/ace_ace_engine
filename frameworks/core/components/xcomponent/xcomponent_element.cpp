...
void XComponentElement::SetNewComponent(const RefPtr<Component>& newComponent)
{
    ...
    auto xcomponentComponent = AceType::DynamicCast<XComponentComponent>(newComponent);
    if (xcomponentComponent) {
        name_ = xcomponentComponent->GetName();
        Element::SetNewComponent(xcomponentComponent);
    }
}
} // namespace OHOS::Ace

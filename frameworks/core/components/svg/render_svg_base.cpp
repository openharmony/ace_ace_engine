...
RefPtr<RenderSvg> FindRootSvgNode(RefPtr<RenderNode> parent, RefPtr<RenderSvgBase>& rootSvgNode)
{
    auto root = AceType::DynamicCast<RenderSvg>(rootSvgNode);
    if (root != nullptr) {
        return root;
    }
...
Offset RenderSvgBase::GetTransformOffset(bool isRoot)
{
    ...
    return Offset(x, y) + GetGlobalOffset();
}
...
void RenderSvgBase::SetPresentationAttrs(const RefPtr<SvgBaseDeclaration>& baseDeclaration)
{
    if (baseDeclaration) {
        ...
        auto& maskId = baseDeclaration->GetMaskId();
        if (maskId.size() > 6) {
            ...
            if (start != std::string::npos) {
                ...
                if (end != std::string::npos) {
                    maskId_ = maskId.substr(start, end - start);
                }
            }
        }
    }
...
{
    return SvgTransform::SetProperty(type, from, to, value, transformAttrs_);
}
...
void RenderSvgBase::AddMaskToRoot(const std::string& id, const RefPtr<RenderSvgBase>& mask)
{
    if (!id.empty() && mask != nullptr) {
        ...
        if (rootSvg != nullptr) {
            rootSvg->AddMaskNode(id, mask);
        }
    }
}

void RenderSvgBase::AddPatternToRoot(const std::string& id, const RefPtr<RenderSvgBase>& mask)
{
    return AddMaskToRoot(id, mask);
}

...
void FlutterRenderText::Paint(RenderContext& context, const Offset& offset)
{
    ...
    switch (textStyle_.GetTextAlign()) {
        ...
        case TextAlign::START:
            paragraph_->Paint(canvas->canvas(), paintOffset.GetX(), paintOffset.GetY());
            break;
        case TextAlign::RIGHT:
        case TextAlign::END:
            paragraph_->Paint(
                canvas->canvas(), paintOffset.GetX() - (textRealWidth - GetLayoutSize().Width()), paintOffset.GetY());
            break;
        case TextAlign::CENTER:
            paragraph_->Paint(canvas->canvas(), paintOffset.GetX() - ((textRealWidth - GetLayoutSize().Width()) / 2.0),
                paintOffset.GetY());
            break;
...
bool FlutterRenderText::CheckMeasureFlag()
{
    ...
    if (!needMeasure_) {
        ...
        if (NearEqual(paragraphMaxWidth, lastLayoutMaxWidth_) && NearEqual(paragraphMinWidth, lastLayoutMinWidth_) &&
            NearEqual(paragraphMaxHeight, lastLayoutMaxHeight_) &&
            NearEqual(paragraphMinHeight, lastLayoutMinHeight_)) {
            // Max-lines is not auto and width constrains not changed.
...
Size FlutterRenderText::GetSize()
{
    ...
    return Size(text_->GetMaxWidthLayout() ? paragraphNewWidth_ : std::ceil(paragraphNewWidth_),
        std::min(

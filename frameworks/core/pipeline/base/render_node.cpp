...
void RenderNode::DumpTree(int32_t depth)
{
    ...
    if (DumpLog::GetInstance().GetDumpFile()) {
        ...
        DumpLog::GetInstance().AddDesc(std::string("ContentSize: ").append(GetContentSize().ToString()));
...
void RenderNode::CheckIfNeedUpdateTouchRect()
{
    ...
    if (!GetTouchRect().IsWrappedBy(parent->GetTouchRect())) {
    ...
    TouchTestResult& result)
{
    ...
    for (auto iter = children.rbegin(); iter != children.rend(); ++iter) {
        ...
        if (child->InterceptTouchEvent() && GetTouchRect().IsInRegion(transformPoint)) {
            dispatchSuccess = true;
            break;
        }
    }
...
double RenderNode::NormalizePercentToPx(const Dimension& dimension, bool isVertical) const
{
    ...
    if (positionParam_.type == PositionType::OFFSET) {
        ...
        parentSize = parent->GetLayoutParam().GetMaxSize();
    }
